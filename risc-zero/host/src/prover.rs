...
impl ProverConfig {
    /// Creates a new `ProverConfig` from a static ELF slice and its
    /// pre-computed Image ID (generated by `risc0-build` at compile time).
    pub const fn new(elf: &'static [u8], image_id: [u32; 8]) -> Self {
        Self { elf, image_id }
    }
}
